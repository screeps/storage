...
function upgradeDb() {
    var schemaCollection = getOrAddCollection('schema');
    var schemaVersion = schemaCollection.findOne({ _id: 'databaseVersion' });
    if(!schemaVersion) {
      schemaVersion = schemaCollection.insert({_id: 'databaseVersion', value: 1});
    }
    ...
    schemaCollection.update(schemaVersion);
